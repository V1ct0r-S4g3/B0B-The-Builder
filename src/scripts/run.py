...
from src.bot.main import MyBot as CompetitiveBot
from src.config.config import BOT_NAME, BOT_RACE, MAP_POOL, MAP_PATH, OPPONENT_RACE, OPPONENT_DIFFICULTY, REALTIME, REPLAY_SAVE_PATH
...
def load_bot(args):
    ...
    # Set the bot's race and return it
    bot.race = bot_race
    return bot
...
def main():
    ...
    try:
        ...
        # For the installed version, we'll try to use the default map loading
        try:
            print("Loading default SC2 map...")
            map_obj = sc2.maps.get(map_name)
        except Exception as e:
            print(f"Error loading map: {e}")
            print("Make sure the map files are in the correct SC2 Maps directory.")
            print(f"Expected location: {os.path.join(os.getenv('SC2PATH', 'SC2PATH not set'), 'Maps')}")
            return 1
        ...
        run_game(
            map_obj,
            [Bot(bot.race, bot), Computer(opponent_race, difficulty)],
